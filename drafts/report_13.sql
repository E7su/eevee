...
--------------------------------------//13//---------------------------------------
CREATE OR REPLACE FORCE VIEW JIRA_READER.V_AVERAGE_WORK
AS
  WITH dates (d) AS (
  ...
  SELECT
    ...
    s."Оформление"                                                     REGISTRATION,
    --   стаж в годах,
    --   для  уволенных -на дату увольнения,
    --   для  работающих -на текущую дату
    ROUND(AVG(MONTHS_BETWEEN(s."Работа до", s."Дата приёма") / 12), 2) "AVG_DURATION, YEARS"
  FROM dates
    LEFT JOIN (
                SELECT
                  emp."Дата приёма",
